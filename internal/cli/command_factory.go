...
func (factory *CommandFactory) Run(cmd *cobra.Command) {
	if err := cmd.Execute(); err != nil {
		...
		if e, ok := err.(CommandSuggester); ok {
			logs = append(logs, terminal.NewFollowupLog(terminal.CommandMessage, e.SuggestedCommands()))
		}
		if e, ok := err.(LinkReferrer); ok {
			logs = append(logs, terminal.NewFollowupLog(terminal.CommandMessage, e.ReferenceLinks()))
		}
