...
echo "uploading the following JSON file to S3 bucket 'stitch-clis/versions/cloud-prod/CURRENT':\n"
echo "$CURRENT\n"
...
echo "$CURRENT" | aws s3 cp - 's3://stitch-clis/versions/cloud-prod/CURRENT' --content-type 'application/json' --acl 'public-read'
